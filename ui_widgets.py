# The contents of this file are the work of splondike:
# https://github.com/splondike/talon_ui_helper
...
def render_text(canvas, formatted_text, x, y):
    ...
    for i, line in enumerate(formatted_text["output_lines"]):
        calc_y = y + (i + 1) * formatted_text["line_height"]
        canvas.draw_text(line, x, calc_y, formatted_text["paint"])


def layout_text(
    text: str, paint: Paint, max_width: int
    ...
    return (
        ...
        {"line_height": line_height, "paint": paint, "output_lines": output_lines},
    )
